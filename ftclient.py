...
from os import path	# path.isfile() checks if a file already exists before creating one
...
"""
RecData(controlSocket, dataPort): Creates the data connection socket and receives data from the server.
It first checks for any error messages from the control connection then starts pulling in data from the data
connection. It will display an error message, list directory contents, or transfer a file as appropriate.
Pre: Socket with the control connection (for receiving error messages if applicable) and a port to open
a data connection on.
Post: Display error, directory, or "transfer complete" as appropriate. New file is added. Both sockets are closed.
"""
